    ...
    paypal.Buttons({
        createOrder: function(data, actions) {
            return actions.order.create({
                purchase_units: [{
                    amount: {
                        value: '<?php echo isset($total) ? $total : 0; ?>' // إجمالي المبلغ المدفوع
                    }
                }]
            });
        },
        onApprove: function(data, actions) {
            return actions.order.capture().then(function(details) {
                let amountPaid = '<?php echo isset($total) ? $total : 0; ?>'; // إجمالي المبلغ المدفوع
                let transactionID = details.id; // رقم المعاملة من PayPal
                
                // تحويل المستخدم إلى صفحة النجاح مع المبلغ ورقم المعاملة
                window.location.href = "payment_sucess.php?amount=" + amountPaid + "&txn_id=" + transactionID;
            });
        }
