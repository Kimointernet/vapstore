	...
	if($count == 1){
			...
			if (isset($_COOKIE["product_list"])) {
				...
				for ($i=0; $i < count($product_list); $i++) { 
					...
					$verify_cart = "SELECT id FROM cart 
			WHERE user_id = $_SESSION[uid] AND p_id = ".$product_list[$i];
					...
					if(mysqli_num_rows($result) < 1){
						...
						$delete_existing_product = "DELETE FROM cart WHERE user_id = -1 
						AND ip_add = '$ip_add' AND p_id = ".$product_list[$i];
